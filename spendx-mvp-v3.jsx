...
const T = {
  ...
  cryptoWallet: { en: "Crypto Wallet", ru: "Крипто-кошелёк", uk: "Крипто-гаманець", de: "Krypto-Wallet", es: "Monedero crypto", tr: "Kripto cüzdan" },
  ...
  issuanceFee: { en: "issuance fee", ru: "комиссия выпуска", uk: "комісія випуску", de: "Ausgabegebühr", es: "tarifa de emisión", tr: "düzenleme ücreti" },
  ...
  off15: { en: "15% off - first order", ru: "Скидка 15% - первый заказ", uk: "Знижка 15% - перше замовлення", de: "15% Rabatt - Erstbestellung", es: "15% descuento - primer pedido", tr: "15% indirim - ilk sipariş" },
  ...
  <div className="card-3d" onClick={onFlip} style={{ height: compact ? 175 : 190 }}>
    <div className={`card-3d-inner ${flipped ? "flipped" : ""}`} style={{ width: "100%", height: "100%" }}>
      <div className="card-face glow-border" style={{
        ...
        {/* Card background: custom skin or circuit pattern */}
        {skinUrl ? (
          <div style={{ position: "absolute", inset: 0, borderRadius: "inherit", backgroundImage: `url(${skinUrl})`, backgroundSize: "cover", backgroundPosition: "center" }} />
        ) : (
          <CircuitBg seed={cd.id || 1} color={cd.color} />
        )}
        {/* Shine animation */}
        <div style={{ position: "absolute", top: 0, left: "-100%", width: "50%", height: "100%", background: "linear-gradient(90deg, transparent, rgba(255,255,255,.04), transparent)", animation: "card-shine 5s ease infinite", pointerEvents: "none", zIndex: 1 }} />
        {/* Change skin button */}
        {onChangeSkin && !locked && (
          <div onClick={e => { e.stopPropagation(); onChangeSkin(); }} style={{
            position: "absolute", top: 10, right: 10, zIndex: 12, width: 26, height: 26, borderRadius: "50%",
            background: "rgba(0,0,0,.35)", backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)",
            display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer",
            border: "1px solid rgba(255,255,255,.12)",
          }}>
            <Ico d="M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z" size={12} color="rgba(255,255,255,.7)" />
          </div>
        )}
        {/* Content */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", position: "relative", zIndex: 2 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <Badge text={t("active")} color={C.green} />
            </div>
...
/* ─── TUTORIAL OVERLAY ─── */
const TutorialOverlay = ({ steps, welcomeTitle, welcomeDesc, doneTitle, doneDesc, onComplete, scrollRef, lang, setLang }) => {
  const t = useT();
  ...
  const measure = (idx) => {
    ...
    const cr = container?.getBoundingClientRect() || { top: 0, left: 0 };
    ...
    if (scrollRef?.current) {
      ...
      const elTop = er.top - cr.top + st;
      if (elTop < st || elTop + er.height > st + vh) {
        scrollRef.current.scrollTo({ top: Math.max(0, elTop - 80), behavior: "smooth" });
        setTimeout(() => measure(idx), 350);
      }
    }
  };
  ...
  if (phase === "welcome") return (
    <div style={{ position: "absolute", inset: 0, zIndex: 500, background: "rgba(5,10,20,.92)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 30px", animation: "fade-up .5s ease", overflow: "auto" }}>
      {/* Wordmark logo - same as auth screen */}
      ...
      {/* Language selector - first priority */}
      ...
      {/* Start Tour + Skip */}
      <PillBtn bg={C.yellow} color={C.navy} onClick={() => goStep(0)} full style={{ maxWidth: 260, padding: "14px 24px", fontSize: 15 }}>
        {t("tutStartTour")}
      </PillBtn>
      ...
      <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", zIndex: 501 }}>
        <defs>
          <mask id="tut-mask">
            ...
            {rect && <rect x={rect.x} y={rect.y} width={rect.w} height={rect.h} rx="20" fill="black" style={{ transition: "all .4s cubic-bezier(.4,0,.2,1)" }} />}
          </mask>
        </defs>
        <rect width="100%" height="100%" fill="rgba(5,10,20,.82)" mask="url(#tut-mask)" />
      </svg>
      {/* Highlight ring */}
      {rect && <div style={{ position: "absolute", left: rect.x, top: rect.y, width: rect.w, height: rect.h, border: `2px solid ${C.yellow}50`, borderRadius: 20, boxShadow: `0 0 20px ${C.yellow}15, inset 0 0 20px ${C.yellow}05`, zIndex: 502, pointerEvents: "none", transition: "all .4s cubic-bezier(.4,0,.2,1)", animation: "glow-breathe 2s ease infinite" }} />}
      {/* Tooltip */}
      {s && <div key={step} style={{
        position: "absolute", zIndex: 510, left: 20, right: 20,
        ...(s.pos === "above" && rect ? { bottom: `calc(100% - ${rect.y - 12}px)` } : {}),
...
const HomeScreen = ({ onNav, onSheet, activeCard, hidden, setHidden, setActiveCard, showToast, myCards, lockedCards, cardSkins, openSkinPicker, lang, setLang, openLangDD }) => {
      ...
      <div style={{ padding: "0 20px" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <Glass className="glass-btn" onClick={openSupport} style={{ flex: 1, padding: "14px 10px", display: "flex", flexDirection: "column", alignItems: "center", gap: 8, cursor: "pointer", border: `1px solid ${C.cardBorder}44` }}>
            ...
            <span style={{ color: C.silver, fontSize: 11, fontWeight: 600, fontFamily: "DM Sans, sans-serif" }}>Support</span>
          </Glass>
          <Glass className="glass-btn" onClick={() => onSheet("faq")} style={{ flex: 1, padding: "14px 10px", display: "flex", flexDirection: "column", alignItems: "center", gap: 8, cursor: "pointer", border: `1px solid ${C.cardBorder}44` }}>
            ...
            <span style={{ color: C.silver, fontSize: 11, fontWeight: 600, fontFamily: "DM Sans, sans-serif" }}>FAQ</span>
          </Glass>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ padding: "0 0 20px", overflowX: "hidden" }}>
      ...
      <div className="s3" style={{ display: "flex", gap: 8, padding: "0 20px", marginBottom: 22 }}>
...
/* ─── ALL CARDS SHEET ─── */
const CARD_TYPES = [
  { id: 1, tier: "Essential", last4: "4582", balance: "$2,150.00", status: "Active", color: "#6b7280", price: "$25", priceDisc: "$21.25", dailyLimit: "$2,500", monthlyLimit: "$75,000", topupFee: "3.5%", feeNum: 3.5, tx: [
  ...
  { id: 2, tier: "Plus", last4: "8891", balance: "$430.00", status: "Active", color: "#0ea5e9", price: "$35", priceDisc: "$29.75", dailyLimit: "$5,000", monthlyLimit: "$150,000", topupFee: "3%", feeNum: 3, tx: [
  ...
  { id: 3, tier: "Prime", last4: "7203", balance: "$1,870.50", status: "Active", color: C.accent, price: "$75", priceDisc: "$63.75", dailyLimit: "$20,000", monthlyLimit: "$600,000", topupFee: "2.5%", feeNum: 2.5, tx: [
  ...
  { id: 4, tier: "Supreme", last4: "9910", balance: "$12,400.00", status: "Active", color: "#111827", price: "$150", priceDisc: "$127.50", dailyLimit: "$100,000", monthlyLimit: "$3,000,000", topupFee: "2%", feeNum: 2, tx: [
  ...
  { id: 5, tier: "Business", last4: "3301", balance: "$0.00", status: "Active", color: "#1e293b", price: "$150", priceDisc: "$127.50", dailyLimit: "$100,000", monthlyLimit: "$3,000,000", topupFee: "2%", feeNum: 2, googleOnly: true, tx: [
    ...
    <div style={{ padding: 20 }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", marginTop: 24 }}>
        <button className="outline-btn" onClick={() => { onClose(); onSheet && onSheet("shop"); }} style={{
          ...
          <Ico d={ic.topup} size={18} color={C.muted} /> Order New Card
        </button>
      </div>
    </div>
  </div>
...
const CardScreen = ({ activeCard, setActiveCard, hidden, setHidden, lockedCards, setLockedCards, showToast, kycStatus, myCards, onSheet, cardSkins, openSkinPicker, lang, setLang, openLangDD }) => {
  ...
  if (!myCards || myCards.length === 0 || !activeCard) return (
    <div style={{ padding: "70px 20px 20px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
      ...
      <div style={{ color: C.muted, fontSize: 13, fontFamily: "DM Sans, sans-serif", marginBottom: 24, lineHeight: 1.5 }}>
        Choose from 4 card tiers with different<br/>limits and fees to match your needs
      </div>
      <PillBtn bg={C.yellow} color={C.navy} full onClick={() => onSheet("shop")} style={{ maxWidth: 260 }}>
        <Ico d={ic.shop} size={16} color={C.navy} sw={2} /> Browse Card Plans
      </PillBtn>
    </div>
  );
    ...
    <div style={{ padding: "0 0 20px", overflowX: "hidden" }}>
      ...
      <div className="s3" style={{ padding: "0 20px", marginBottom: 14 }}>
        <Glass style={{ padding: 18 }}>
          ...
          {[
            { label: t("daily"), val: activeCard.dailyLimit, used: 43, spent: (parseFloat(activeCard.dailyLimit.replace(/[$K,]/g, "")) * 1000 * 0.43) },
            { label: t("monthly"), val: activeCard.monthlyLimit, used: 18, spent: (parseFloat(activeCard.monthlyLimit.replace(/[$K,]/g, "")) * 1000 * 0.18) },
            ...
            <div key={l.label} className={l.used !== null ? "limit-bar" : ""} style={{ marginBottom: 0, ...(l.used === null ? { padding: "10px 14px", margin: "0 -14px", display: "flex", justifyContent: "space-between" } : {}) }}>
              ...
              {l.used !== null && (<>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
                  <span style={{ color: C.muted, fontSize: 10, fontFamily: "DM Sans, sans-serif" }}>${l.spent.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} spent</span>
      ...
      <div style={{ padding: 20 }}>
        ...
        <Glass className="s4" style={{ padding: 16, marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ color: C.muted, fontSize: 12, fontFamily: "DM Sans, sans-serif", textDecoration: "line-through" }}>$25</span>
              <span style={{ color: C.green, fontSize: 16, fontWeight: 700, fontFamily: "DM Sans, sans-serif" }}>$19.99</span>
            </div>
          </div>
          <div style={{ color: C.muted, fontSize: 10, fontFamily: "DM Sans, sans-serif", lineHeight: 1.5 }}>
            Pre-order now and lock in the early-bird price. You will be charged when the card ships. Full refund available until shipping.
          </div>
        </Glass>
        ...
        <div className="s5">
          {reserved ? (
            ...
            <PillBtn bg={C.yellow} color={C.navy} full onClick={() => setReserved(true)}>
              <Ico d={ic.card} size={16} color={C.navy} sw={2} /> Pre-order Now - $19.99
            </PillBtn>
          )}
        </div>
      </div>
          ...
          <Glass className="glass-btn" onClick={() => setPreorderOpen(true)} style={{
            ...
            <div style={{ padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                ...
                <div style={{ color: C.muted, fontSize: 10, fontFamily: "DM Sans, sans-serif", marginTop: 2 }}>{t("earlyBird")} $19.99</div>
              </div>
...
const TopUpSheet = ({ onClose, activeCard }) => {
  const t = useT();
  ...
  return (
    <div className="sheet-anim" style={{ background: C.navy, minHeight: "100%", paddingBottom: 20 }}>
      ...
      <div style={{ padding: "10px 20px" }}>
        ...
        <Glass style={{ padding: "0 16px", marginBottom: 12 }}>
          {isIn && (
            <div className="menu-row" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "13px 0", borderBottom: `1px solid ${C.gridSep}33`, cursor: "pointer" }}>
          ...
          <div className="menu-row" onClick={openSupport} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "13px 0", cursor: "pointer" }}>
...
const HistorySheet = ({ onClose, activeCard }) => {
  const t = useT();
  ...
  const allTx = CARD_TYPES.flatMap(c => c.tx.map(t => ({ ...t, tier: c.tier, last4: c.last4, color: c.color, cardId: c.id })))
    .sort((a, b) => {
  ...
  const currentCard = filter === "all" ? null : CARD_TYPES.find(c => c.id === filter);
  ...
  if (selectedTx) {
    const ci = CARD_TYPES.find(c => c.id === (selectedTx.cardId || activeCard.id)) || activeCard;
    return <TxDetailSheet tx={selectedTx} cardInfo={ci} onClose={() => setSelectedTx(null)} />;
  }
  const headerLabel = filter === "all" ? "All Cards" : `${currentCard.tier} ** ${currentCard.last4}`;
      ...
      <div style={{ padding: "10px 20px" }}>
        ...
        <div style={{ marginBottom: 14 }}>
          <div className="outline-btn" onClick={() => setDropOpen(!dropOpen)} style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              {filter !== "all" && <div style={{ width: 8, height: 8, borderRadius: "50%", background: currentCard.color, boxShadow: `0 0 4px ${currentCard.color}60` }} />}
            ...
            <Glass style={{ marginTop: 6, overflow: "hidden", animation: "fade-up .2s ease both" }}>
              ...
              {CARD_TYPES.map((c, i) => (
                <div key={c.id} className="menu-row" onClick={() => { setFilter(c.id); setDropOpen(false); }} style={{
                  display: "flex", alignItems: "center", gap: 10, padding: "11px 14px",
                  borderBottom: i < CARD_TYPES.length - 1 ? `1px solid ${C.gridSep}22` : "none",
                  cursor: "pointer",
...
const CardShopSheet = ({ onClose, orderCard, ownedCards, kycStatus, setKycStatus, showToast }) => {
  ...
  if (ordered) return (
    <div className="sheet-anim" style={{ background: C.navy, minHeight: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 30 }}>
      ...
      <div style={{ color: C.muted, fontSize: 13, fontFamily: "DM Sans, sans-serif", textAlign: "center", marginBottom: 24 }}>Your virtual card is being issued</div>
      <PillBtn bg={C.yellow} color={C.navy} full onClick={onClose}>
        Go to My Cards
      </PillBtn>
    </div>
  ...
  if (payingCard) {
    ...
    <div className="sheet-anim" style={{ background: C.navy, minHeight: "100%", paddingBottom: 20 }}>
      <div style={{ display: "flex", alignItems: "center", padding: "14px 20px", borderBottom: `1px solid ${C.gridSep}44` }}>
        ...
        <span style={{ flex: 1, textAlign: "center", color: C.white, fontSize: 15, fontWeight: 600, fontFamily: "DM Sans, sans-serif" }}>Pay for {payingCard.tier}</span>
        <div style={{ width: 34 }} />
      </div>
      <div style={{ padding: 20 }}>
        {/* Amount */}
        <Glass style={{ padding: 16, marginBottom: 14, textAlign: "center", borderRadius: 20, border: `1px solid ${C.yellow}25` }}>
          <div style={{ color: C.muted, fontSize: 11, fontFamily: "DM Sans, sans-serif", textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>Card issuance fee</div>
        ...
        <div className="s1" style={{ textAlign: "center", marginBottom: 16 }}>
          <div onClick={() => {
            ...
            {payAddrCopied
              ? <><Ico d={ic.check} size={14} color={C.yellow} sw={2.5} /><span style={{ color: C.yellow, fontSize: 13, fontWeight: 700, fontFamily: "DM Sans, sans-serif" }}>Address Copied!</span></>
        ...
        <Glass className="s2" style={{ overflow: "hidden", marginBottom: 10 }}>
          {/* Token selector */}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "13px 16px", borderBottom: `1px solid ${C.gridSep}33` }}>
            <span style={{ color: C.muted, fontSize: 13, fontFamily: "DM Sans, sans-serif" }}>Pay with token</span>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              {["USDT", "USDC"].map(c => (
          ...
          <div style={{ borderBottom: `1px solid ${C.gridSep}33` }}>
            <div style={{ color: C.muted, fontSize: 10, fontFamily: "DM Sans, sans-serif", textTransform: "uppercase", letterSpacing: 1, padding: "10px 16px 4px" }}>Network</div>
            {PAY_NETS.map((n, i) => (
          ...
          {/* Amount to send */}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "13px 16px" }}>
            <span style={{ color: C.muted, fontSize: 13, fontFamily: "DM Sans, sans-serif" }}>Send exactly</span>
            <div onClick={() => {
        ...
        <Glass className="s3" style={{ padding: "10px 14px", marginBottom: 18, display: "flex", alignItems: "flex-start", gap: 8, border: `1px solid ${C.amber}20`, background: `${C.amber}06` }}>
          ...
          <span style={{ color: C.muted, fontSize: 11, lineHeight: 1.5, fontFamily: "DM Sans, sans-serif" }}>
            Send only <span style={{ color: C.silver, fontWeight: 600 }}>{payCoin}</span> on <span style={{ color: C.silver, fontWeight: 600 }}>{selPayNet.name}</span>. Card will be issued after payment confirmation.
          </span>
        </Glass>
      ...
      <div style={{ padding: 20 }}>
        ...
        {kycStatus !== "verified" && (
          <Glass className="glass-btn" onClick={() => setKycOpen(true)} style={{
            ...
            border: `1.5px solid ${C.yellow}40`, background: `${C.yellow}08`,
            boxShadow: `0 0 16px ${C.yellow}12`,
          }}>
        ...
        {CARD_TYPES.map((card, i) => {
          ...
          return (
            <Glass key={card.id} className={`s${Math.min(i + 1, 5)}`} style={{
              ...
              border: `1px solid ${owned ? C.green + "30" : C.cardBorder + "44"}`,
              opacity: owned ? 0.7 : 1,
            }}>
              {/* Card visual with circuit skin */}
              <div style={{
                ...
                <CircuitBg seed={card.id + 200} color={card.color} />
                ...
                <div style={{ position: "relative", zIndex: 1 }}>
                  <div style={{ color: (CARD_SKINS[card.color] || {}).light ? "rgba(40,45,70,.5)" : C.vivid, fontSize: 9, fontFamily: "DM Sans, sans-serif", letterSpacing: 2, textTransform: "uppercase" }}>SpendX</div>
                  <div style={{ color: (CARD_SKINS[card.color] || {}).light ? "#2d3255" : C.white, fontSize: 22, fontWeight: 800, fontFamily: "DM Sans, sans-serif" }}>{card.tier}</div>
                </div>
                <div style={{ position: "relative", zIndex: 1, textAlign: "right" }}>
                  <div style={{ color: C.muted, fontSize: 12, fontFamily: "DM Sans, sans-serif", textDecoration: "line-through" }}>{card.price}</div>
                  <div style={{ color: C.green, fontSize: 18, fontWeight: 700, fontFamily: "DM Sans, sans-serif" }}>{card.priceDisc}</div>
                  <div style={{ color: C.muted, fontSize: 9, fontFamily: "DM Sans, sans-serif" }}>issuance fee</div>
                </div>
              </div>
                ...
                <div style={{ marginTop: 10 }}>
                  {owned ? (
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "10px 0" }}>
                      <Ico d={ic.check} size={14} color={C.green} sw={2.5} />
                      <span style={{ color: C.green, fontSize: 13, fontWeight: 600, fontFamily: "DM Sans, sans-serif" }}>Owned</span>
                    </div>
                  ) : kycStatus !== "verified" ? (
                    <PillBtn bg={C.yellow} color={C.navy} full onClick={() => setKycOpen(true)} style={{ padding: "12px 0", fontSize: 13 }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 30 }} onClick={() => setConfirmCard(null)}>
          <Glass onClick={e => e.stopPropagation()} style={{ padding: 24, maxWidth: 320, width: "100%", borderRadius: 24, border: `1px solid ${C.yellow}30`, animation: "fade-up .25s ease both" }}>
            <div style={{ textAlign: "center", marginBottom: 16 }}>
              <div style={{ color: C.white, fontSize: 18, fontWeight: 700, fontFamily: "DM Sans, sans-serif" }}>Order {confirmCard.tier}?</div>
              <div style={{ color: C.muted, fontSize: 12, fontFamily: "DM Sans, sans-serif", marginTop: 6 }}>Issuance fee: <span style={{ color: C.green, fontWeight: 700 }}>{confirmCard.priceDisc}</span></div>
            </div>
            {[
...
/* ─── FAQ SHEET ─── */
const FAQ_DATA = [
  { q: "What is SpendX?", a: "SpendX LLC is a fintech company at the intersection of crypto infrastructure, stablecoin settlements, and digital payments. The company issues crypto-linked payment cards (virtual and physical) and builds an integrated ecosystem connecting digital assets, banking instruments, and Web3 in one unified interface. Incorporated in Dubai, UAE. License No. 2528382.01.", cat: "general" },
  { q: "What card types and plans are available?", a: "SpendX offers both physical and virtual cards across five plans: Essential ($25), Plus ($35), Prime ($75), Supreme ($150), and Business ($150). Each tier has its own daily and monthly spending limits and top-up fee.", cat: "card" },
  { q: "Where can I use my SpendX Card?", a: "Anywhere Mastercard or Visa is accepted - online and in-store, at 100M+ merchants worldwide.", cat: "card" },
  { q: "How do I top up my SpendX Card?", a: "Send USDT or USDC directly to your personal deposit address. Supported networks: Tron (TRC20), BNB Smart Chain (BEP20), and Solana. Funds convert to USD and land on your card balance.", cat: "card" },
  { q: "How do I activate my SpendX Card?", a: "Your card is activated automatically after completing KYC verification and purchasing the card. No manual steps required.", cat: "card" },
  { q: "Can I spend in different currencies?", a: "The SpendX Card is issued in USD, but you can pay in any currency worldwide. Exchange rates are determined by the banks involved in the transaction.", cat: "card" },
  { q: "Do I need to complete KYC?", a: "Yes - SpendX is fully KYC-compliant. You need a valid ID card, passport, or residence permit. It is a one-time process required before card issuance.", cat: "kyc" },
  { q: "How long does KYC verification take?", a: "Most verifications complete quickly. If selected for manual review it may take up to 72 hours. Your virtual card is issued immediately after approval.", cat: "kyc" },
  { q: "How to stay safe online?", a: "Many similarly named websites exist. Use only our official site: spend-x.com. All official social media accounts carry verification badges.", cat: "kyc" },
  { q: "What does it cost to top up my card?", a: "Top-up fees range from 2.0% to 3.5% depending on your plan. Always shown before confirming - no hidden charges.", cat: "fees" },
  { q: "Are there hidden fees when spending?", a: "Many services charge up to 8.33% hidden fees at purchase. SpendX charges only the stated top-up fee - your full balance is always usable.", cat: "fees" },
];
...
const FAQSheet = ({ onClose }) => {
  ...
  return (
    <div className="sheet-anim" style={{ background: C.navy, minHeight: "100%", paddingBottom: 20 }}>
      <div style={{ display: "flex", alignItems: "center", padding: "14px 20px", borderBottom: `1px solid ${C.gridSep}44` }}>
        <CircleBtn icon="back" color={C.panelLight} iconColor={C.silver} size={34} shadow={false} onClick={onClose} />
        <span style={{ flex: 1, textAlign: "center", color: C.white, fontSize: 15, fontWeight: 600, fontFamily: "DM Sans, sans-serif" }}>FAQ</span>
        <div style={{ width: 34 }} />
      </div>
      <div style={{ padding: 20 }}>
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          ...
          <div style={{ color: C.white, fontSize: 16, fontWeight: 700, fontFamily: "DM Sans, sans-serif" }}>Frequently Asked Questions</div>
        </div>
    ...
    <div className="sheet-anim" style={{ background: C.navy, minHeight: "100%", paddingBottom: 20 }}>
      ...
      <div style={{ padding: 20 }}>
        ...
        {kycStatus === "not_started" && (
          <>
            <div style={{ color: C.muted, fontSize: 10, fontFamily: "DM Sans, sans-serif", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>What you'll need</div>
            <Glass style={{ padding: "4px 0", marginBottom: 18, overflow: "hidden" }}>
              {[
                { icon: "📄", text: "Valid passport or national ID" },
                { icon: "🤳", text: "Selfie for liveness check" },
                { icon: "🏠", text: "Proof of address (utility bill, bank statement)" },
              ].map((r, i) => (
                <div key={i} style={{
                  display: "flex", alignItems: "center", gap: 12, padding: "12px 16px",
                  borderBottom: i < 2 ? `1px solid ${C.gridSep}33` : "none",
                }}>
            ...
            <PillBtn bg={C.yellow} color={C.navy} full onClick={() => {
              ...
              // In real app: redirect to KYC provider
            }}>
              Start Verification
            </PillBtn>
        ...
        {kycStatus === "pending" && (
          <>
            <Glass style={{ padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, border: `1px solid ${C.amber}25`, background: `${C.amber}06`, marginBottom: 16 }}>
              ...
              <div>
                <div style={{ color: C.silver, fontSize: 13, fontWeight: 600, fontFamily: "DM Sans, sans-serif" }}>Verification in progress</div>
                <div style={{ color: C.muted, fontSize: 11, fontFamily: "DM Sans, sans-serif", marginTop: 2 }}>Usually 5-15 minutes. We'll notify you.</div>
              </div>
            </Glass>

            {/* Demo: simulate approval */}
            <OutlineBtn onClick={() => { setKycStatus("verified"); showToast("KYC Verified!"); }} style={{ justifyContent: "center" }}>
              <Ico d={ic.check} size={14} color={C.green} /> Simulate Approval (demo)
            </OutlineBtn>
          </>
        )}
        ...
        {kycStatus === "verified" && (
          <>
            <Glass style={{ padding: "14px 16px", display: "flex", alignItems: "center", gap: 10, border: `1px solid ${C.green}25`, background: `${C.green}06`, marginBottom: 20 }}>
              ...
              <span style={{ color: C.silver, fontSize: 13, fontFamily: "DM Sans, sans-serif" }}>Full access unlocked. You can order any card tier.</span>
            </Glass>
            <PillBtn bg={C.yellow} color={C.navy} full onClick={onClose}>
...
export default function SpendXMVP3() {
  ...
  const [ownedCards, setOwnedCards] = useState([]); // array of card IDs
  ...
  const myCards = CARD_TYPES.filter(c => ownedCards.includes(c.id));
  const orderCard = (cardId) => {
    if (kycStatus !== "verified") { showToast("Complete KYC first"); return false; }
    if (ownedCards.includes(cardId)) { showToast("Card already owned"); return false; }
    setOwnedCards(prev => [...prev, cardId]);
    const card = CARD_TYPES.find(c => c.id === cardId);
    if (!activeCard) setActiveCard(card);
    showToast(`${card.tier} card ordered!`);
    return true;
  };
  ...
  const TUTORIAL_EMPTY = [
    ...
    { id: "lang-switcher", title: tl("tutLang"), desc: tl("tutLangDesc"), pos: "below", pad: 12 },
  ...
  const TUTORIAL_HOME = [
    { id: "balance", title: tl("tutTotalBalance"), desc: tl("tutTotalBalanceDesc"), pos: "below", pad: 10 },
    ...
    { id: "recent-activity", title: tl("tutRecentActivity"), desc: tl("tutRecentActivityDesc"), pos: "above", pad: 6 },
    { id: "live-prices", title: tl("tutPricesHelp"), desc: tl("tutPricesHelpDesc"), pos: "above", pad: 6 },
    { id: "tab-bar", title: tl("tutNav"), desc: tl("tutNavDesc"), pos: "above", pad: 6 },
  ];
  ...
  const screens = {
    home: <HomeScreen onNav={nav} onSheet={setSheet} activeCard={activeCard} hidden={hidden} setHidden={setHidden} setActiveCard={setActiveCard} showToast={showToast} myCards={myCards} lockedCards={lockedCards} cardSkins={cardSkins} openSkinPicker={openSkinPicker} lang={lang} setLang={setLang} openLangDD={openLangDD} />,
    card: <CardScreen activeCard={activeCard} setActiveCard={setActiveCard} hidden={hidden} setHidden={setHidden} lockedCards={lockedCards} setLockedCards={setLockedCards} showToast={showToast} kycStatus={kycStatus} myCards={myCards} onSheet={setSheet} cardSkins={cardSkins} openSkinPicker={openSkinPicker} lang={lang} setLang={setLang} openLangDD={openLangDD} />,
    settings: <SettingsScreen lang={lang} setLang={setLang} showToast={showToast} kycStatus={kycStatus} setKycStatus={setKycStatus} />,
  };
  const sheets = {
    topup: activeCard ? <TopUpSheet onClose={() => setSheet(null)} activeCard={activeCard} /> : null,
    history: activeCard ? <HistorySheet onClose={() => setSheet(null)} activeCard={activeCard} /> : null,

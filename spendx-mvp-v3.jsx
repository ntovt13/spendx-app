...
const TutorialOverlay = ({ steps, welcomeTitle, welcomeDesc, doneTitle, doneDesc, onComplete, scrollRef, lang, setLang, showLangPicker = true }) => {
  const t = useT();
  const [phase, setPhase] = useState(showLangPicker ? "welcome" : "steps"); // welcome | steps | done
  ...
  const measure = (idx) => {
    ...
    const isVisible = er.top >= srRect.top - 20 && er.bottom <= srRect.bottom + 20;

    if (!isVisible && sr) {
      // Scroll to element first, then re-measure
      const elOffsetTop = el.offsetTop || 0;
      let parent = el.offsetParent;
      let totalOffset = elOffsetTop;
      while (parent && parent !== sr) {
        totalOffset += parent.offsetTop || 0;
        parent = parent.offsetParent;
      }
      sr.scrollTo({ top: Math.max(0, totalOffset - 80), behavior: "smooth" });
      setTimeout(() => measure(idx), 450);
      return;
    }
  ...
  // Initial scroll to top + measure for first step
  useEffect(() => {
    if (phase === "steps" && step === 0) {
      const sr = scrollRef?.current;
      if (sr) {
        sr.scrollTo({ top: 0, behavior: "smooth" });
        setTimeout(() => measure(0), 500);
      } else {
        setTimeout(() => measure(0), 150);
      }
    }
  }, [phase]);

  const goStep = (idx) => {
    setRect(null); // clear highlight for smooth transition
    setStep(idx);
    setPhase("steps");
  ...
  if (phase === "welcome") return (
    <div style={{ position: "absolute", inset: 0, zIndex: 500, background: "rgba(5,10,20,.92)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 30px", animation: "fade-up .5s ease", overflow: "auto" }}>
      ...
      {/* Language selector - only on first tutorial */}
      {showLangPicker && (
  ...
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 500, pointerEvents: "none" }}>
      {/* SVG mask spotlight */}
      <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", zIndex: 501, pointerEvents: "none" }}>
        <defs>
          <mask id="tut-mask">
      ...
      {s && <div key={step} style={{
        position: "absolute", zIndex: 510, left: 20, right: 20, pointerEvents: "auto",
      ...
      {/* Prevent clicks on background elements but allow scroll */}
...
const HomeScreen = ({ onNav, onSheet, activeCard, hidden, setHidden, setActiveCard, showToast, myCards, lockedCards, cardSkins, openSkinPicker, lang, setLang, openLangDD }) => {
  ...
  return (
    <div style={{ padding: "0 0 20px", overflowX: "hidden" }}>
      ...
      <div data-tut="quick-actions" className="s3" style={{ display: "flex", gap: 8, padding: "0 20px", marginBottom: 22 }}>
            ...
            <Glass key={card.id} className={`s${Math.min(i + 1, 5)}`} style={{
              ...
              <div style={{
                ...
                <div style={{ position: "relative", zIndex: 1, textAlign: "right" }}>
                  ...
                  <div style={{ color: C.muted, fontSize: 9, fontFamily: "DM Sans, sans-serif" }}>{t("issuanceFee")}</div>
                </div>
              </div>
                ...
                <div style={{ marginTop: 10 }}>
                  {owned ? (
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "10px 0" }}>
                      <Ico d={ic.check} size={14} color={C.green} sw={2.5} />
                      <span style={{ color: C.green, fontSize: 13, fontWeight: 600, fontFamily: "DM Sans, sans-serif" }}>{t("ownedLabel")}</span>
                    </div>
                  ) : kycStatus !== "verified" ? (
...
export default function SpendXMVP3() {
  ...
  const nav = t => { setTab(t); setSheet(null); if (scrollRef.current) scrollRef.current.scrollTop = 0; };
  const openSheet = (s) => { setSheet(s); if (scrollRef.current) scrollRef.current.scrollTop = 0; };
